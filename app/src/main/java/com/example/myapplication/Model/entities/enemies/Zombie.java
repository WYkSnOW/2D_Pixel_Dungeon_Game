...
public class Zombie extends Character {
    ...
    private void updateMove(double delta, MapManager mapManager, PointF playerPos) {
        ...
        float baseSpeed = (float) delta * 200;
        ...
        if (distance <= 4 * GameConstants.Sprite.SIZE) { //怪物检测范围，单位为格
            chaseMoveMode(distanceX, distanceY, baseSpeed, gameMap, facePoint);
        } else {
            defaultMoveMode(gameMap, facePoint, baseSpeed);
        }
    ...
    private void chaseMoveMode(
        ...
        float deltaX = xSpeed * baseSpeed; //移动镜头而不是角色
        float deltaY = ySpeed * baseSpeed; //因镜头需与角色相反的方向移动，即乘以-1
    ...
    private void defaultMoveMode(GameMap gameMap, float facePoint, float baseSpeed) {
